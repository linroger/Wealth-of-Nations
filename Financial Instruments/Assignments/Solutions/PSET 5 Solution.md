...
![](ht tps://storage.simpletex.cn/vi ew/fyyg666isOQVV hplkPU976LrXANn ionPx)
...
2. The relationship is convex. The "second derivative" is positive..
...
the value of such new portfolio is $V_{u}^{RP}=\triangle_{u}\cdot S_{u}+B_{u}=1.000\cdot110+(-95.2381)=$ 14.7619 that is exactly equal to the liquidation value of the portfolio that was set up at period $i=0$ .We can do the same excercise for node (2,1) and we see that
...
after dividend) will be $ S_{d}=\:S_{0}\cdot d\cdot(1-y)=\:100\cdot\frac{1}{1.1}\cdot(1-0.05 )=\ :86.3636$ What happens next? The t ock pricewill still have  gross return of either $u$ OI d. Figure (5)  shows the new tree for thestock price.
...
<table>
	<tbody>
		<tr>
			<th colspan="2">If input increases...</th>
		</tr>
		<tr>
			<th>Change in call pricez</th>
			<th>Change in put price</th>
		</tr>
		<tr>
			<td>Stock Price</td…		<td>$\uparrow$</td>
			<td>$\downarrow$</td>
		</tr>
		<tr>
			<td>Strike Price</td>
