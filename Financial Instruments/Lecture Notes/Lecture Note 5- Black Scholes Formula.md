...
2. From now on, *rebalance* the portfolio, to make sure that at every t, the portfolio has a position in stocks given by$$∆t = −N(−d_{1,t})$$
...
2. Today make$(\text{Put Option Premium} −P_t) > 0$.
...
3. **THETA**: SENSITIVITY OF OPTION TO PASSAGE OF TIME T RISKS IN OPTIONS AND THE GREEKS$$\Theta=\frac{d\text{ Option Price}}{ d \mathrm{~}t}=\text{Long ugly formula}$$
...
7. **Rho**: Change in option price due to a change in interest rate r
...
5. **VEGA**: CHANGE IN OPTION PRICE DUE TO A CHANGE IN VOLATILITY Σ EXAMPLE OF RISK MANAGEMENT PROBLEM$$\text{Vega}=\frac{d\text{ Option Price}}{ d \mathrm{~}\sigma}=S\sqrt{T}N^{\prime}(d_1)>0$$
