...
**Risk Neutral Pricing: A Recipe**
 ...
 3. Price any derivative security using the formula:$$\text{Price of Derivative Security} = E'_0[e^{-rT} \times \text{Payoff at } T]$$
...
Conditions to approximate the expected return$\mu$and variance$\sigma^2$:
 ...
 2. Variance:$E[(S_{i+1}/S_i / e^{\mu h})^2] = \sigma^2 \times h$leading to a specific relation for$u, d,$and$q$.
...
- Steps include:
	 ...
	 2. Moving backward on the tree with$V_{i,j} = e^{-rh} \times E^*[V_{I+1,j}]$.
