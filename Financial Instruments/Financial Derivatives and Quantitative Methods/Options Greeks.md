...
We have from the Black-Scholes formula that the price of a call option depends on the price of the underlying asset, S, the strike priceK, the time to maturity, T ,  the interest rate,  r,  and the volatility,  σ. Write this as

A functionC=C (S,  K,  T,  r,  σ). Then using a first-order approximation,  we have
...
This shows the effect of varying each of the parameters, S, T$ ， $T$ ， 0 by small amounts $\delta S$ $\delta T$ ， $\delta r$ and δσ,  but withK fixed. The same will be true for any option or portfolio of options. Thus,  if Π=C (S,  T,  r,  σ) is the value of the option or portfolio of options,  then the value of the portfolio after a small change in the parameters is given by 3
