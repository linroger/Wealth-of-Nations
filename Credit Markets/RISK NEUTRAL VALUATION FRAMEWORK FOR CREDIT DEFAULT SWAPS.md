...
2. The risk free valuation formula simplifies to: $PV(t) = \mathbb{E}\left[\int_t^{\infty} DF(t, s) \cdot dCF(s) | \mathcal{F}_t\right] \quad (4)$
...
$PV_{CDS_DL}(t) = \mathbb{E}\left[\int_t^T DF(t, s) \cdot dDL(s) | \tau > t\right] \quad (16)$

$= \mathbb{E}\left[\int_t^T (1 - R) \cdot DF(t, s) \cdot d\mathbf{1}_{{\tau \leq s}} | \tau > t\right] \quad (17)$

$= \int_t^T (1 - R) \cdot DF(t, s) \cdot d\mathbb{E}\left[\mathbf{1}_{{\tau \leq s}} | \tau > t\right] \quad (18)$

$= (1 - R) \cdot \int_t^T DF(t, s) \cdot d\mathbb{P}(\tau \leq s | \tau > t) \quad (19)$

$= (1 - R) \cdot \int_t^T DF(t, s) \cdot dDP(t, s). \quad (20)$
...
2. Hazard rate curves $h(t, \cdot)$ defined implicitly from Survival Probability curves: $SP(t, s) = \exp\left[-\int_t^s h(t, u) du\right], \quad 0 \leq t \leq s. \quad (24)$
3. Or defined explicitly as: $h(t, s) = -\frac{\partial}{\partial s} \ln[SP(t, s)] = -\frac{1}{SP(t, s)} \cdot \frac{\partial SP}{\partial s}(t, s). \quad (25)$
...
$PV_{CDS_DL}(t) = (1 - R) \cdot \int_t^T DF(t, s) \cdot dDP(t, s) \quad (27)$
$= (1 - R) \cdot \int_t^T DF(t, s) \cdot d[1 - SP(t, s)] \quad (28)$
$= - (1 - R) \cdot \int_t^T DF(t, s) \cdot dSP(t, s) \quad (29)$
$= (1 - R) \cdot \int_t^T h(t, s) \cdot DF(t, s) \cdot SP(t, s) \cdot ds. \quad (30)$

## NUMERICAL QUADRATURE OF CDS DEFAULT/LOSS LEG

$PV_{CDS_DL}(t) = (1 - R) \cdot \int_t^T h(t, s) \cdot DF(t, s) \cdot SP(t, s) \cdot ds$
$= \sum_{i=1}^n (1 - R) \cdot \int_{T_{i-1}}^{T_i} h(t, s) \cdot DF(t, s) \cdot SP(t, s) \cdot ds \quad (31)$
$\simeq \sum_{i=1}^n (1 - R) \cdot h(t, T_i) \cdot \Delta T_i \cdot DF(t, T_i) \cdot SP(t, T_i). \quad (32)$

## CDS VALUATION (RECEIVING PREMIUM $C$, PAYING DEFAULT/LOSS LEG)

$PV_{CDS}(t) = PV_{CDS_PL}(t) - PV_{CDS_DL}(t) \quad (33)$
$= \sum_{i=1}^n c \cdot DF(t, T_i) \cdot SP(t, T_i)$
$- (1 - R) \cdot \int_t^T h(t, s) \cdot DF(t, s) \cdot SP(t, s) \cdot ds$
$= \sum_{i=1}^n [c - (1 - R) \cdot h(t, T_i)] \cdot \Delta T_i \cdot DF(t, T_i) \cdot SP(t, T_i). \quad (34)$

## CDS DURATION AND PARSPREAD
1. CDS Duration defined as the "Unit" 1bp Premium Leg PV: $Duration(t):= \sum_{i=1}^n \Delta T_i \cdot DF(t, T_i) \cdot SP(t, T_i) \quad (35)$
2. CDS Premium Leg valuation via Duration: $PV_{CDS_PL}(t) = c \cdot Duration(t) \quad (36)$
3. ParSpread defined from CDS Default Leg and Duration: $PV_{CDS_DL}(t) = ParSpread(t) \cdot Duration(t) \quad (37)$
4. CDS valuation: $PV_{CDS}(t) = [c - ParSpread(t)] \cdot Duration(t) \quad (38)$

## CDS DURATION AND PARSPREAD FORMULAS
1. CDS trades "At Par" when ParSpread matches the coupon: $$PV_{CDS}(t) = 0 \Leftrightarrow ParSpread(t) = c \quad (39)$$
2. CDS ParSpread measures the credit risk of the CDS contract: $$ParSpread(t) \cong (1 - R) \cdot h \quad (40)$$
3. CDS CS01 defined as $\Delta PV$ for -1bp change in ParSpread: $$CS01_{CDS}(t, s):= PV_{CDS}(t, s - 1bp) - PV_{CDS}(t, s)$$$$\cong [c - s + 1bp] \cdot Duration(t) - [c - s] \cdot Duration(t) = Duration(t) \quad (41)$$
1. Cumulative Future Cash Flows: $CF(t), \quad t \geq 0 \quad (1)$ This formula represents the cumulative future cash flows of an instrument over time. It captures all the cash inflows and outflows associated with the instrument from the current time $t$ onwards.
2. Present Value: $$PV(t) = B(t) \cdot \mathbb{E}\left[\int_t^{\infty} B(s)^{-1} \cdot dCF(s) | \mathcal{F}_t\right] \quad (2)$$
...
3. Deterministic Discount Factor: $$DF(t, s) = B(t) \cdot \mathbb{E}\left[B(s)^{-1}|\mathcal{F}_t\right] = e^{-\int_t^s r_u du}, \quad 0 \leq t \leq s \quad (3)$$
...
4. Simplified Present Value (Deterministic Interest Rates): $$PV(t) = \mathbb{E}\left[\int_t^{\infty} DF(t, s) \cdot dCF(s) | \mathcal{F}_t\right] \quad (4)$$
...
5. CDS Premium Leg Cash Flows: $$PL(s):= \sum_{i=1}^n c_i \cdot \mathbf{1}_{{s \geq T_i}} \cdot \mathbf{1}_{{\tau > T_i}} \quad (6)$$
...
6. Issuer Survival Probability: $$SP(t, s):= \mathbb{P}(\tau > s | \tau > t), \quad 0 \leq t \leq s, \quad (7)$$
...
7. Issuer Default Probability: $$DP(t, s):= \mathbb{P}(\tau \leq s | \tau > t) = 1 - SP(t, s). \quad (8)$$
...
8. Default Leg Cash Flows: $$DL(s):= (1 - R) \cdot \mathbf{1}_{{\tau \leq s}} \quad (15)$$
...
9. Present Value of Default Leg (using Fubini's theorem): $$PV_{CDS_DL}(t) = (1 - R) \cdot \int_t^T DF(t, s) \cdot dDP(t, s). \quad (20)$$
...
10. Hazard Rates: $$SP(t, s) = \exp\left[-\int_t^s h(t, u) du\right], \quad 0 \leq t \leq s. \quad (24)$$
...
11. Present Value of Default Leg (using Hazard Rates): $$PV_{CDS_DL}(t) = (1 - R) \cdot \int_t^T h(t, s) \cdot DF(t, s) \cdot SP(t, s) \cdot ds. \quad (30)$$
...
12. CDS Valuation: $$PV_{CDS}(t) = PV_{CDS_PL}(t) - PV_{CDS_DL}(t) \quad (33)$$
...
13. CDS Duration: $$Duration(t):= \sum_{i=1}^n \Delta T_i \cdot DF(t, T_i) \cdot SP(t, T_i) \quad (35)$$
...
14. CDS ParSpread: $$PV_{CDS_DL}(t) = ParSpread(t) \cdot Duration(t) \quad (37)$$
...
15. CDS CS01: $$CS01_{CDS}(t, s):= PV_{CDS}(t, s - 1bp) - PV_{CDS}(t, s) \quad (41)$$
