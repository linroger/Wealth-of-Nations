...
1. CDS Premium Leg valuation via Duration: $PV_{CDS_PL}(t) = c \cdot Duration(t) \quad (36)$
1. ParSpread defined from CDS Default Leg and Duration: $PV_{CDS_DL}(t) = ParSpread(t) \cdot Duration(t) \quad (37)$
1. CDS valuation: $PV_{CDS}(t) = [c - ParSpread(t)] \cdot Duration(t) \quad (38)$
...
1. CDS ParSpread measures the credit risk of the CDS contract: $$ParSpread(t) \cong (1 - R) \cdot h \quad (40)$$
1. CDS CS01 defined as $\Delta PV$ for -1bp change in ParSpread: $$CS01_{CDS}(t,  s):= PV_{CDS}(t,  s - 1bp) - PV_{CDS}(t,  s)$$$$\cong [c - s + 1bp] \cdot Duration(t) - [c - s] \cdot Duration(t) = Duration(t) \quad (41)$$
...
1. Present Value: $$PV(t) = B(t) \cdot \mathbb{E}\left[\int_t^{\infty} B(s)^{-1} \cdot dCF(s) | \mathcal{F}_t\right] \quad (2)$$
...
1. Deterministic Discount Factor: $$DF(t,  s) = B(t) \cdot \mathbb{E}\left[B(s)^{-1}|\mathcal{F}_t\right] = e^{-\int_t^s r_u du},  \quad 0 \leq t \leq s \quad (3)$$
...
1. Simplified Present Value (Deterministic Interest Rates): $$PV(t) = \mathbb{E}\left[\int_t^{\infty} DF(t,  s) \cdot dCF(s) | \mathcal{F}_t\right] \quad (4)$$
...
1. CDS Premium Leg Cash Flows: $$PL(s):= \sum_{i=1}^n c_i \cdot \mathbf{1}_{{s \geq T_i}} \cdot \mathbf{1}_{{\tau > T_i}} \quad (6)$$
...
1. Issuer Survival Probability: $$SP(t,  s):= \mathbb{P}(\tau > s | \tau > t),  \quad 0 \leq t \leq s,  \quad (7)$$
...
1. Issuer Default Probability: $$DP(t,  s):= \mathbb{P}(\tau \leq s | \tau > t) = 1 - SP(t,  s). \quad (8)$$
...
1. Default Leg Cash Flows: $$DL(s):= (1 - R) \cdot \mathbf{1}_{{\tau \leq s}} \quad (15)$$
