...
4. The present value is obtained as: $$PV(t) = B(t) \cdot \mathbb{E}\left[\int_t^{\infty} B(s)^{-1} \cdot dCF(s) | \mathcal{F}_t\right] \tag{1}$$

## SIMPLE CASE: DETERMINISTIC INTEREST RATES

For the simple case of deterministic interest rates:

1. The "time value of money" at time $t$ for time $s$ is captured in the calibrated discount factor curve: $$DF(t, s) = B(t) \cdot \mathbb{E}\left[B(s)^{-1}|\mathcal{F}_t\right] = e^{-\int_t^s r_u du}, \quad 0 \leq t \leq s \tag{1}$$
2. The risk free valuation formula simplifies to: $PV(t) = \mathbb{E}\left[\int_t^{\infty} DF(t, s) \cdot dCF(s) | \mathcal{F}_t\right] \tag{1}$
...
1. Quarterly coupon payments until maturity $T = T_n$: ${c_i, T_i}_{i=1…n}, \quad 0 \leq t < T_1 < … < T_n \tag{1}$
2. We denote the random/stochastic issuer default time by: $\tau \in [0, \infty)$
3. Cumulative Premium Leg cash flows have stochastic dependence on $\tau$: $$PL(s):= \sum_{i=1}^n c_i \cdot \mathbf{1}_{{s \geq T_i}} \cdot \mathbf{1}_{{\tau > T_i}} \tag{1}$$
...
4. Market implied issuer survival & default probabilities: $$SP(t, s):= \mathbb{P}(\tau > s | \tau > t), \quad 0 \leq t \leq s, \tag{1}$$ $$DP(t, s):= \mathbb{P}(\tau \leq s | \tau > t) = 1 - SP(t, s). \tag{1}$$
...
4. Default Leg cash flows until time $s$ are given by: $DL(s):= (1 - R) \cdot \mathbf{1}_{{\tau \leq s}} \tag{1}$

## VALUATION OF CDS DEFAULT/LOSS LEG (USING FUBINI)

The present value of the CDS default/loss leg is:

$PV_{CDS_DL}(t) = \mathbb{E}\left[\int_t^T DF(t, s) \cdot dDL(s) | \tau > t\right] \tag{1}$

$= \mathbb{E}\left[\int_t^T (1 - R) \cdot DF(t, s) \cdot d\mathbf{1}_{{\tau \leq s}} | \tau > t\right] \tag{1}$

$= \int_t^T (1 - R) \cdot DF(t, s) \cdot d\mathbb{E}\left[\mathbf{1}_{{\tau \leq s}} | \tau > t\right] \tag{1}$

$= (1 - R) \cdot \int_t^T DF(t, s) \cdot d\mathbb{P}(\tau \leq s | \tau > t) \tag{1}$

$= (1 - R) \cdot \int_t^T DF(t, s) \cdot dDP(t, s). \tag{1}$

The intuitive interpretation is that the default leg present value at the (random) issuer default time is:

$PV_{CDS_DL}(t) = \int_t^T (1 - R) \cdot DF(t, s) \cdot dDP(t, s) \tag{1}$
...
2. Hazard rate curves $h(t, \cdot)$ defined implicitly from Survival Probability curves: $SP(t, s) = \exp\left[-\int_t^s h(t, u) du\right], \quad 0 \leq t \leq s. \tag{1}$
3. Or defined explicitly as: $h(t, s) = -\frac{\partial}{\partial s} \ln[SP(t, s)] = -\frac{1}{SP(t, s)} \cdot \frac{\partial SP}{\partial s}(t, s). \tag{1}$
...
$\mathbb{P}(s < \tau \leq s + \varepsilon | \tau > s, \tau > t) \tag{1}$
...
$PV_{CDS_DL}(t) = (1 - R) \cdot \int_t^T DF(t, s) \cdot dDP(t, s) \tag{1}$

$= (1 - R) \cdot \int_t^T DF(t, s) \cdot d[1 - SP(t, s)] \tag{1}$

$= - (1 - R) \cdot \int_t^T DF(t, s) \cdot dSP(t, s) \tag{1}$

$= (1 - R) \cdot \int_t^T h(t, s) \cdot DF(t, s) \cdot SP(t, s) \cdot ds. \tag{1}$

## NUMERICAL QUADRATURE OF CDS DEFAULT/LOSS LEG

$PV_{CDS_DL}(t) = (1 - R) \cdot \int_t^T h(t, s) \cdot DF(t, s) \cdot SP(t, s) \cdot ds$

$= \sum_{i=1}^n (1 - R) \cdot \int_{T_{i-1}}^{T_i} h(t, s) \cdot DF(t, s) \cdot SP(t, s) \cdot ds \tag{1}$

$\simeq \sum_{i=1}^n (1 - R) \cdot h(t, T_i) \cdot \Delta T_i \cdot DF(t, T_i) \cdot SP(t, T_i). \tag{1}$

## CDS VALUATION (RECEIVING PREMIUM $C$, PAYING DEFAULT/LOSS LEG)

$PV_{CDS}(t) = PV_{CDS_PL}(t) - PV_{CDS_DL}(t) \tag{1}$

$= \sum_{i=1}^n c \cdot DF(t, T_i) \cdot SP(t, T_i)$

$- (1 - R) \cdot \int_t^T h(t, s) \cdot DF(t, s) \cdot SP(t, s) \cdot ds$

$= \sum_{i=1}^n [c - (1 - R) \cdot h(t, T_i)] \cdot \Delta T_i \cdot DF(t, T_i) \cdot SP(t, T_i). \tag{1}$

## CDS DURATION AND PARSPREAD

1. CDS Duration defined as the "Unit" 1bp Premium Leg PV: $Duration(t):= \sum_{i=1}^n \Delta T_i \cdot DF(t, T_i) \cdot SP(t, T_i) \tag{1}$
2. CDS Premium Leg valuation via Duration: $PV_{CDS_PL}(t) = c \cdot Duration(t) \tag{1}$
3. ParSpread defined from CDS Default Leg and Duration: $PV_{CDS_DL}(t) = ParSpread(t) \cdot Duration(t) \tag{1}$
4. CDS valuation: $PV_{CDS}(t) = [c - ParSpread(t)] \cdot Duration(t) \tag{1}$

## CDS DURATION AND PARSPREAD FORMULAS

1. CDS trades "At Par" when ParSpread matches the coupon: $$PV_{CDS}(t) = 0 \Leftrightarrow ParSpread(t) = c \tag{1}$$
2. CDS ParSpread measures the credit risk of the CDS contract: $$ParSpread(t) \cong (1 - R) \cdot h \tag{1}$$
3. CDS CS01 defined as $\Delta PV$ for -1bp change in ParSpread: $$CS01_{CDS}(t, s):= PV_{CDS}(t, s - 1bp) - PV_{CDS}(t, s)$$ $$\cong [c - s + 1bp] \cdot Duration(t) - [c - s] \cdot Duration(t) = Duration(t) \tag{1}$$
...
2. Present Value: $$PV(t) = B(t) \cdot \mathbb{E}\left[\int_t^{\infty} B(s)^{-1} \cdot dCF(s) | \mathcal{F}_t\right] \tag{1}$$
...
3. Deterministic Discount Factor: $$DF(t, s) = B(t) \cdot \mathbb{E}\left[B(s)^{-1}|\mathcal{F}_t\right] = e^{-\int_t^s r_u du}, \quad 0 \leq t \leq s \tag{1}$$
...
4. Simplified Present Value (Deterministic Interest Rates): $$PV(t) = \mathbb{E}\left[\int_t^{\infty} DF(t, s) \cdot dCF(s) | \mathcal{F}_t\right] \tag{1}$$
...
5. CDS Premium Leg Cash Flows: $$PL(s):= \sum_{i=1}^n c_i \cdot \mathbf{1}_{{s \geq T_i}} \cdot \mathbf{1}_{{\tau > T_i}} \tag{1}$$
...
6. Issuer Survival Probability: $$SP(t, s):= \mathbb{P}(\tau > s | \tau > t), \quad 0 \leq t \leq s, \tag{1}$$
...
7. Issuer Default Probability: $$DP(t, s):= \mathbb{P}(\tau \leq s | \tau > t) = 1 - SP(t, s). \tag{1}$$
...
8. Default Leg Cash Flows: $$DL(s):= (1 - R) \cdot \mathbf{1}_{{\tau \leq s}} \tag{1}$$
...
9. Present Value of Default Leg (using Fubini's theorem): $$PV_{CDS_DL}(t) = (1 - R) \cdot \int_t^T DF(t, s) \cdot dDP(t, s). \tag{1}$$
...
10. Hazard Rates: $$SP(t, s) = \exp\left[-\int_t^s h(t, u) du\right], \quad 0 \leq t \leq s. \tag{1}$$
...
11. Present Value of Default Leg (using Hazard Rates): $$PV_{CDS_DL}(t) = (1 - R) \cdot \int_t^T h(t, s) \cdot DF(t, s) \cdot SP(t, s) \cdot ds. \tag{1}$$
...
12. CDS Valuation: $$PV_{CDS}(t) = PV_{CDS_PL}(t) - PV_{CDS_DL}(t) \tag{1}$$
...
13. CDS Duration: $$Duration(t):= \sum_{i=1}^n \Delta T_i \cdot DF(t, T_i) \cdot SP(t, T_i) \tag{1}$$
...
14. CDS ParSpread: $$PV_{CDS_DL}(t) = ParSpread(t) \cdot Duration(t) \tag{1}$$
...
15. CDS CS01: $$CS01_{CDS}(t, s):= PV_{CDS}(t, s - 1bp) - PV_{CDS}(t, s) \tag{1}$$
