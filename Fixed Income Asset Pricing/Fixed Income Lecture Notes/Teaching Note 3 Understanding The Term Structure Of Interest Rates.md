---
aliases: [FORWARD RATES AND TERM STRUCTURE, Term Structure of Interest Rates, Term Structure]
...
- The current long-term yield$y_t(n + 1)$depends on
	...
	3. A convexity term due to the nonlinear relation that exists between the yield$y_{t+1}(n)$and the price$$Z_{t+1}(n) = e^{−y_{t+1}(n)×n}$$
...
-  Why [DiD](Lecture%2013-%20Difference-In-Differences%20(Part%202%20Of%202).md) the CAPM beta change over time?
...
- Many risk premia are accounted for in bond pricing:
	  ...
	  2. **Expected inflation risk premium**:$\gamma \sigma_g \rho_{\pi g} \sigma_{\pi}$. If$\rho_{g, \pi} < 0 \Rightarrow r(t, T) \uparrow$
