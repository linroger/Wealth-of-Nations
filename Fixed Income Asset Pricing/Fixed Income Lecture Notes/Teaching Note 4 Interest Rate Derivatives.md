...
- As interest rates changed between January and April, so [DiD](Lecture%2013-%20Difference-In-Differences%20(Part%202%20Of%202).md) the *value* of the FRA.
...
2. Obtain the dollar value of the remaining caplet$T_i$as the difference between the cap price for$T_i$(obtained in Step 1) and the sum of caplets up to$T_{i-1}$
...
3. Find the (forward) volatility$σ^{Fwd}_f(T_j)$such that
...
2. The dollar value of the$T_2$caplet is then

$$\text{Dollar value of$T_2$caplet} =Cap(T_{2})-Caplet\left(T_{1},r_{K},\sigma_{f}^{Fwd}(T_{1})\right)$$

$=$$\$0.1059-\$0.0273=\$0.0786$

3. Use the Black formula again to find the (forward) volatility$σ^{Fwd}_f(T_2)$such that
...
2. The dollar value of the$T_3$caplet is then
