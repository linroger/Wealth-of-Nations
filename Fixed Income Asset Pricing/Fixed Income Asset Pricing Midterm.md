---
cssclasses: academia
aliases: [Fixed Income Asset Pricing Bus 35130 Spring 2019 John Heaton and Oleg Sydyak,  Alias_139_Fixed Income Asset Pricing Midterm.md,  Alias_140_Fixed Income Asset Pricing Midterm.md]
tags: tag_example
...
(2.C)(7 points) The market expects that because of a tightening of [[Lecture 7-Risk and Return of Bonds#7.6 Asset price reactions to monetary policy surprises|monetary policy]],  the one-period interest rate at time $i=1$ will be $4\%$.What is the implied market price of risk? Comment on its sign and its economic meaning.
...
There is an arbitrage in the quoted forward rate $F(9~months,  1~year)~=~1.45\%$ compared to [[A Guide to the Front End and Basis Swap Markets#London Interbank Offered Rate (LIBOR)|LIBOR]] rates. Discuss.

True: Yes,  there is an arbitrage.To find out,  we need to compute the forward rate implied by [[A Guide to the Front End and Basis Swap Markets#London Interbank Offered Rate (LIBOR)|LIBOR]] quotes.And for this,  we only need the “zero coupon bond prices implied by [[A Guide to the Front End and Basis Swap Markets#London Interbank Offered Rate (LIBOR)|LIBOR]] curve
...
The forwardrate implied by the [[A Guide to the Front End and Basis Swap Markets#London Interbank Offered Rate (LIBOR)|LIBOR]] is far larger than the quoted FRA.Accord ing to the model,  there is an arbitrage opportunity.
...
and short the security that makes us pay $F^{Quoted}$.The latter is simple,  we simply enter into a [[A Guide to the Front End and Basis Swap Markets#Forward Rate Agreements (FRAs) Overview|FRA]] with a counterparty to receive [[A Guide to the Front End and Basis Swap Markets#London Interbank Offered Rate (LIBOR)|LIBOR]] and pay the quoted [[A Guide to the Front End and Basis Swap Markets#Forward Rate Agreements (FRAs) Overview|FRA]] That is,  denoting $T_{1}=0.75$ and $T_{2}=1=T_{1}+\Delta$ and $\Delta=0.25$,  we have at $T_{2}$

$$\text{Cash Flow at}T_2\text{from FRA}=\left(LIBOR(0.75)-F^{Quoted}(T_1,  T_2)\right)\Delta $$

How do we get $FLIBOR?$ We simply have to borrow and lend at [[A Guide to the Front End and Basis Swap Markets#London Interbank Offered Rate (LIBOR)|LIBOR]] in the appropriate direction. In particular,  by just looking at the formula for $F^{LIBOR}$,  we se thathi sgiventh the ae teadtet roe $\frac{Z(0,  0.75)}{Z(0,  1)}-1]$ yea seroed reis follows
...
At time $T_{1}=0.75$，we have to pay 1 from the short position in $Z(0,  T_{1})$.Assume we borrow at whatever [[A Guide to the Front End and Basis Swap Markets#London Interbank Offered Rate (LIBOR)|LIBOR]] rate will be available,  $LIBOR(T_{1})$,  for the following three month
...
Summing the Net Cash Flow from Trading to the Cash Flow from [[A Guide to the Front End and Basis Swap Markets#Forward Rate Agreements (FRAs) Overview|FRA]] position,  we obtain
...
True: A receiver swaption is an option to enter into a swap at a given maturity $T_{0}$ and received the fixed rate $C$ against [[A Guide to the Front End and Basis Swap Markets#London Interbank Offered Rate (LIBOR)|LIBOR]] in a swap with tenor 7.The owner of a swaption will only exercise when the market swap rate at time $T_{O}$ is below the strike swap rate $\bar{c}$. In other words,  the option owner will exercise at $T_{0}$ if and only if the value of the swap with swap rate $\mathbb{C}$ is positive. The payoff of the swaption is thus
...
which is the value of a call option on a coupon bond whose discounts $Z(T_{O},  T_{j})$ are determined by the [[A Guide to the Front End and Basis Swap Markets#London Interbank Offered Rate (LIBOR)|LIBOR]] curve at $T_{O}$
