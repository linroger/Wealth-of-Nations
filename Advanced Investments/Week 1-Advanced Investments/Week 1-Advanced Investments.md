...
We will spend a good portion of the time in this course on learning how we can assess properties of risk and return of diferent asset classes. Knowing these risk-return properties, and their economic drivers, is a prerequisite for implementing optimal portfolio choice models
...
where $\gamma>0$ is the relative risk aversion coefficient. For $\gamma=1$ ,it may seem that $U(W)$ is undefined, but in fact the limit for $\gamma\rightarrow1$ does exist and it is simply the natural logarithm of $W$ .1 So we use

$$U(W)=\log(W)\quad\text{if}\quad\gamma=1$$
